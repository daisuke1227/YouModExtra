...
static NSArray <YouModAudioOutputFormat *> *YouModAudioOutputFormats(void) {
    ...
    dispatch_once(&onceToken, ^{
        formats = @[
            ...
            YouModAudioOutputFormatMake(@"dsd", @"DSD", @"Not supported", @"dsf", nil, NO, NO),
        ];
    });
    return formats;
}
...
static NSString *YouModAudioOutputSubtitle(YouModAudioOutputFormat *outputFormat) {
    if (!outputFormat.supported) return [NSString stringWithFormat:@"%@ (unavailable)", outputFormat.subtitle ?: @""];
    BOOL needsFFmpeg = !outputFormat.passthroughWhenCompatible;
    NSString *suffix = needsFFmpeg ? @"requires FFmpeg" : @"fast if compatible";
    return outputFormat.subtitle.length ? [NSString stringWithFormat:@"%@ (%@)", outputFormat.subtitle, suffix] : suffix;
}
