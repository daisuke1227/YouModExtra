...
#define SponsorBlockUserID @"SponsorBlockUserID"
#define SponsorBlockEnabled @"SponsorBlockEnabled"
#define SponsorBlockNotifications @"SponsorBlockNotifications"
#define SponsorBlockSegmentMarkers @"SponsorBlockSegmentMarkers"
#define SponsorBlockPlayerButton @"SponsorBlockPlayerButton"
...
static NSArray<NSDictionary *> *YouModSponsorBlockCategories(void) {
    return @[
        ...
        @{ @"id": @"selfpromo", @"title": @"Self-promotion", @"defaultAction": @(YouModSponsorBlockActionSkip), @"color": @1},
        @{ @"id": @"interaction", @"title": @"Interaction", @"defaultAction": @(YouModSponsorBlockActionSkip), @"color": @2},
        @{ @"id": @"intro", @"title": @"Intro", @"defaultAction": @(YouModSponsorBlockActionSkip), @"color": @3},
        @{ @"id": @"outro", @"title": @"Endcards/Credits", @"defaultAction": @(YouModSponsorBlockActionSkip), @"color": @4},
        @{ @"id": @"preview", @"title": @"Preview/Recap", @"defaultAction": @(YouModSponsorBlockActionSkip), @"color": @5},
        @{ @"id": @"music_offtopic", @"title": @"Music off-topic", @"defaultAction": @(YouModSponsorBlockActionSkip), @"color": @6},
        @{ @"id": @"filler", @"title": @"Filler/Jokes", @"defaultAction": @(YouModSponsorBlockActionSkip), @"color": @7},
        @{ @"id": @"poi_highlight", @"title": @"Highlight", @"defaultAction": @(YouModSponsorBlockActionDisplay), @"color": @8},
    ];
}
...
- (void)layoutSubviews {
    ...
    CGFloat currentX = 12.0;

    // SponsorBlock Button
    UIButton *sbButton = objc_getAssociatedObject(self, @selector(YouModSponsorBlockButtonTapped:));
    if (!IS_ENABLED(SponsorBlockEnabled) || !IS_ENABLED(SponsorBlockPlayerButton)) {
        [sbButton removeFromSuperview];
        objc_setAssociatedObject(self, @selector(YouModSponsorBlockButtonTapped:), nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else {
        if (!sbButton) {
            sbButton = [UIButton buttonWithType:UIButtonTypeSystem];
            sbButton.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.55];
            sbButton.tintColor = UIColor.whiteColor;
            sbButton.layer.cornerRadius = 16.0;
            sbButton.titleLabel.font = [UIFont boldSystemFontOfSize:12.0];
            [sbButton setTitle: @"SB" forState:UIControlStateNormal];
            [sbButton addTarget:self action: @selector(YouModSponsorBlockButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
            [self addSubview:sbButton];
            objc_setAssociatedObject(self, @selector(YouModSponsorBlockButtonTapped:), sbButton, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
        sbButton.frame = CGRectMake(currentX, top, 32.0, 32.0);
        [self bringSubviewToFront:sbButton];
        currentX += 40.0;
    }

    // Mute Button
    UIButton *muteButton = objc_getAssociatedObject(self, @selector(YouModMutePlayerButtonTapped:));
    if (!IS_ENABLED(MutePlayerButton)) {
        [muteButton removeFromSuperview];
        objc_setAssociatedObject(self, @selector(YouModMutePlayerButtonTapped:), nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else {
        if (!muteButton) {
            muteButton = [UIButton buttonWithType:UIButtonTypeSystem];
            muteButton.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.55];
            muteButton.tintColor = UIColor.whiteColor;
            muteButton.layer.cornerRadius = 16.0;
            
            float currentVolume = [[AVAudioSession sharedInstance] outputVolume];
            BOOL isMuted = currentVolume == 0.0;
            [muteButton setImage:YouModYTImageNamed(isMuted ? @"yt_outline_volume_off_24pt" : @"yt_outline_volume_up_24pt", isMuted ? @"speaker.slash.fill" : @"speaker.wave.2.fill") forState:UIControlStateNormal];
            
            [muteButton addTarget:self action: @selector(YouModMutePlayerButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
            [self addSubview:muteButton];
            objc_setAssociatedObject(self, @selector(YouModMutePlayerButtonTapped:), muteButton, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
        muteButton.frame = CGRectMake(currentX, top, 32.0, 32.0);
        [self bringSubviewToFront:muteButton];
        currentX += 40.0;
    }

    // Screen Lock Button
    UIButton *lockButton = objc_getAssociatedObject(self, @selector(YouModScreenLockButtonTapped:));
    if (!IS_ENABLED(ScreenLockButton)) {
        [lockButton removeFromSuperview];
        objc_setAssociatedObject(self, @selector(YouModScreenLockButtonTapped:), nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else {
        if (!lockButton) {
            lockButton = [UIButton buttonWithType:UIButtonTypeSystem];
            lockButton.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.55];
            lockButton.tintColor = UIColor.whiteColor;
            lockButton.layer.cornerRadius = 16.0;
            
            BOOL isLocked = [objc_getAssociatedObject(self, @selector(YouModScreenLocked)) boolValue];
            [lockButton setImage:YouModYTImageNamed(isLocked ? @"yt_outline_lock_24pt" : @"yt_outline_unlock_24pt", isLocked ? @"lock.fill" : @"lock.open.fill") forState:UIControlStateNormal];
            
            [lockButton addTarget:self action: @selector(YouModScreenLockButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
            [self addSubview:lockButton];
            objc_setAssociatedObject(self, @selector(YouModScreenLockButtonTapped:), lockButton, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
...
%new
- (void)YouModMutePlayerButtonTapped:(UIButton *)sender {
    ...
    float currentVolume = [[AVAudioSession sharedInstance] outputVolume];
    BOOL isMuted = currentVolume == 0.0;
    static float prevVolume = 0.5;
    if (isMuted) {
        volumeViewSlider.value = prevVolume > 0 ? prevVolume : 0.5;
        [sender setImage:YouModYTImageNamed(@"yt_outline_volume_up_24pt", @"speaker.wave.2.fill") forState:UIControlStateNormal];
        YouModShowToast(@"Unmuted", self);
    } else {
        prevVolume = currentVolume;
        volumeViewSlider.value = 0.0;
        [sender setImage:YouModYTImageNamed(@"yt_outline_volume_off_24pt", @"speaker.slash.fill") forState:UIControlStateNormal];
        YouModShowToast(@"Muted", self);
    }
}

%new
- (void)YouModScreenLockButtonTapped:(UIButton *)sender {
    BOOL isLocked = [objc_getAssociatedObject(self, @selector(YouModScreenLocked)) boolValue];
    isLocked = !isLocked;
    objc_setAssociatedObject(self, @selector(YouModScreenLocked), @(isLocked), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    
    UIView *lockOverlay = objc_getAssociatedObject(self, @selector(YouModLockOverlay));
    if (isLocked) {
        if (!lockOverlay) {
            lockOverlay = [[UIView alloc] initWithFrame:self.bounds];
            lockOverlay.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
            lockOverlay.backgroundColor = [UIColor clearColor];
            UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(YouModLockOverlayTapped:)];
            [lockOverlay addGestureRecognizer:tap];
            [self addSubview:lockOverlay];
            objc_setAssociatedObject(self, @selector(YouModLockOverlay), lockOverlay, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
        [self bringSubviewToFront:lockOverlay];
        [self bringSubviewToFront:sender]; 
        [sender setImage:YouModYTImageNamed(@"yt_outline_lock_24pt", @"lock.fill") forState:UIControlStateNormal];
        YouModShowToast(@"Screen Locked", self);
    } else {
        [lockOverlay removeFromSuperview];
        objc_setAssociatedObject(self, @selector(YouModLockOverlay), nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        [sender setImage:YouModYTImageNamed(@"yt_outline_unlock_24pt", @"lock.open.fill") forState:UIControlStateNormal];
        YouModShowToast(@"Screen Unlocked", self);
    }
}
